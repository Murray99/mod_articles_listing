...
/**
 * Helper for mod_articles_listing
 *
 * @package     Joomla.Site
 * @subpackage  mod_articles_listing
 *
 * @since       1.6
 */
abstract class ModArticlesListingHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		foreach ($items as &$item)
		{
			...
			if ($access || in_array($item->access, $authorised))
			{
				...
				$item->linkText = JText::_('MOD_ARTICLES_LISTING_READMORE');
			}
			else
			{
				$item->link     = JRoute::_('index.php?option=com_users&view=login');
				$item->linkText = JText::_('MOD_ARTICLES_LISTING_READMORE_REGISTER');
			}

			$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_articles_listing.content');
